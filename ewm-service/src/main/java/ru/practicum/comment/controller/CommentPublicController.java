...
public class CommentPublicController {
    ...
    List<CommentResponseDto> getAllComments(@RequestParam(defaultValue = "0") int from,
        ...
        log.info("EWM-SERVICE-public => Запрошены все комментарии from => {}, size => {}", from, size);
    ...
    List<CommentResponseDto> getCommentsByEventId(@Positive @PathVariable long eventId,
                                                  ...
                                                  @RequestParam(defaultValue = "10") int size) {
        log.info("EWM-SERVICE-public => Запрошены все комментарии события по id => {}, from => {}, size => {}",
                eventId, from, size);
